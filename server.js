// PONTE LOGISTICO VERSIONE-DELETE-E-NOTIFICHE
// se non vedi il pulsante ✕ sulle tratte, questo file non e' quello caricato.
...
app.post("/api/ship", (req, res) => {
  ...
  const item = {
    id: DB.seq++, by: by || "?", fromRep: by,
    toRep: by === "Campionario" ? "Produzione" : "Campionario",
    from, to, when: when || "non indicato", note: note || "",
    at: new Date().toLocaleString("it-IT", { timeZone:"Europe/Rome" }),
    ts: Date.now(), departAt: calcolaPartenza(when), votes: [],
  };
...
app.post("/api/deltratta", (req, res) => {
  ...
  const i = DB.tratte.findIndex(x => x.id === trattaId);
  if (i === -1) return res.status(400).json({ error: "tratta non trovata" });
  if (DB.tratte.length <= 1) return res.status(400).json({ error: "serve almeno una tratta" });
  DB.tratte.splice(i, 1);
  persist();
...
app.post("/api/delitem", (req, res) => {
  ...
  if (i === -1) return res.status(400).json({ error: "spedizione non trovata" });
  t.items.splice(i, 1);
  persist();
  broadcast("tratta", { state: DB });
  res.json({ ok: true, state: DB });
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .tratta .del{margin-left:6px;width:20px;height:20px;border-radius:50%;border:none;cursor:pointer;
    background:rgba(255,107,107,.18);color:var(--no);font-size:12px;font-weight:900;line-height:1;
    display:none;align-items:center;justify-content:center;flex-shrink:0}
  .tratta:hover .del{display:flex}
  .tratta .del:hover{background:var(--no);color:#160404}
  .card .delitem{position:absolute;top:12px;right:12px;width:24px;height:24px;border-radius:50%;
    border:none;cursor:pointer;background:rgba(255,107,107,.14);color:var(--no);font-size:13px;
    font-weight:900;display:flex;align-items:center;justify-content:center}
  .card .delitem:hover{background:var(--no);color:#160404}
